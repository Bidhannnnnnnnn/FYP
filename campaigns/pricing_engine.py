...
class PricingEngine:
    ...
    def calculate_price(billboard, slots_data, slot_duration, frequency):
        ...
        for slot in slots_data:
            ...
            # Traffic Density Modifier (if present)
            traffic_multiplier = Decimal(1.0)
            if hasattr(billboard, 'traffic_density'):
                if billboard.traffic_density == 'High':
                    traffic_multiplier = Decimal(1.2)
                elif billboard.traffic_density == 'Low':
            ...
            # Hour Cost = (Hourly Base) * TimeMult * SeasonMult * TrafficMult * DurationMult * FreqMult
            hour_cost = (hourly_base * time_multiplier * seasonality_multiplier * 
                         traffic_multiplier * duration_multiplier * frequency_multiplier)
